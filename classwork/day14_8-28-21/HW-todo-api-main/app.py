...
@app.route("/item/removeall", methods=["DELETE"])
def delete_all_items():

    # Get items from the helper
    res_data = helper.delete_all_items()
    # res_data = {'count': 1, 'items': [('Setting up Flask', 'Not Started')]}

    # Return response. json.dumps breaks down the data where the user can understand.
    response = Response(json.dumps(res_data), mimetype="application/json")
    return response
